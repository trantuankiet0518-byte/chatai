...
export function predictVanHan(chart: TuViEngineResult, targetDateString?: string): VanHanPrediction {
  const effectiveDate = targetDateString || new Date().toISOString().split("T")[0];
  const { year: targetYear, month: targetMonth, day: targetDay } = parseDateInput(effectiveDate);
  ...
  const tuoiAmLich = getTuoiAmLich(birthSolarDateStr, effectiveDate);
