...
export default function LapLaSoVanHanCard({
  ...
  const { lifePalace: life, bodyPalace: body, careerPalace: career } = result;
        ...
        <div className="grid grid-cols-2 gap-2">
          {life && (
            <div className="glass-border-panel-soft rounded-lg p-3">
              ...
              <p className="text-sm font-bold text-on-surface">{getBranchLabel(life.branch)}</p>
              <p className="text-[9px] text-on-surface-variant mt-0.5">
                {life.majorStars.length > 0
                  ? life.majorStars.map((s) => getStarLabel(s.name)).join(", ")
                  : "Vô chính diệu"}
              </p>
            </div>
          )}
          {body && (
            <div className="glass-border-panel-soft rounded-lg p-3">
              ...
              <p className="text-sm font-bold text-on-surface">{getBranchLabel(body.branch)}</p>
              <p className="text-[9px] text-on-surface-variant mt-0.5">
                {getPalaceLabel(body.name)}
              </p>
            </div>
          )}
        </div>
